...
Document.find = function(str,options,callback) {
    ...
    if(options.size) q.size = options.size;
    if(options.from) q.from = options.from;
    if(options.published) q.filter = {
        term: { published:true}
    };

    ec.search(q, function(err,results,eRes) {
        if(err) {
            console.error(err);
            return callback(err);
        }
...
Document.get = function(id,callback) {
    return ec.get('documents',id,callback);
};
...
Document.update = function(id,doc,callback) {
    ...
    ec.index('documents','document',doc,{id:id},callback);
...
Document.add = function(description,fileIds,tags,date,callback) {
    flow.exec(function() {
        ...
        for(var i in tags) {
            Tag.getOrCreate(tags[i], (function(_flow,tagWord,callback) {
                return function(err,words) {
                    if(err) return callback(err);
                    _flow.tags = _flow.tags.concat(words);
                    return callback();
                };
            }(this, tags[i],this.MULTI())));
        }
        for(var i in fileIds) {
